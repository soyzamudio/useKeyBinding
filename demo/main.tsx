  ...
  return <>
    ...
    <main id="main" className="wrap">
      ...
      <section id="getting-started" className="section guide" aria-labelledby="guide-title"><div className="section-heading"><div><div className="eyebrow">02 / GETTING STARTED</div><h2 id="guide-title" tabIndex={-1}>Small API. Short setup.</h2></div><span className="outline-badge">EARLY RELEASE</span></div>
        <div className="guide-grid"><div className="guide-step"><span className="step-number">1</span><h3>Install the hook</h3><p>Add the package to your React app with npm.</p><pre><code>{'npm install use-key-binding'}</code></pre></div><div className="guide-step"><span className="step-number">2</span><h3>Bind something good</h3><p>Import the hook. Pass your keys, then a callback. Here’s the binding to put inside your component.</p><div className="snippet"><button className="copy-button" onClick={copyCode}>{copied ? "Copied ✓" : "Copy example"}</button><pre><code>{bindingCode}</code></pre></div><p className="copy-feedback" role="status">{copyError ? "Clipboard unavailable. Select and copy the code above." : copied ? "Copied a complete React example. Make it yours." : "Use with React 18+ and an ESM-compatible bundler."}</p></div></div>
